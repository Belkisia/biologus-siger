...
<\/script>
  ...
  </body></html>`),b.document.close()}function ne({onSave:t,onCancel:d,titulo:b}){const f=h.useRef(null),u=h.useRef(!1),r=h.useRef(null),o=(l,g)=>{const x=g.getBoundingClientRect(),A=g.width/x.width,_=g.height/x.height;return"touches"in l?{x:(l.touches[0].clientX-x.left)*A,y:(l.touches[0].clientY-x.top)*_}:{x:(l.clientX-x.left)*A,y:(l.clientY-x.top)*_}},n=l=>{l.preventDefault();const g=f.current;g&&(u.current=!0,r.current=o(l,g))},y=l=>{if(l.preventDefault(),!u.current)return;const g=f.current,x=g?.getContext("2d");if(!g||!x||!r.current)return;const A=o(l,g);x.beginPath(),x.moveTo(r.current.x,r.current.y),x.lineTo(A.x,A.y),x.strokeStyle="#000",x.lineWidth=2.5,x.lineCap="round",x.lineJoin="round",x.stroke(),r.current=A},C=()=>{u.current=!1,r.current=null},w=()=>{const l=f.current,g=l?.getContext("2d");!l||!g||g.clearRect(0,0,l.width,l.height)},D=()=>{const l=f.current;l&&t(l.toDataURL("image/png"))};return e.jsxs("div",{className:"flex flex-col gap-3",children:[e.jsx("p",{className:"text-sm font-medium text-center",children:b}),e.jsx("div",{className:"border-2 border-dashed border-border rounded-lg bg-white touch-none",style:{touchAction:"none"},children:e.jsx("canvas",{ref:f,width:600,height:200,className:"w-full rounded-lg",style:{touchAction:"none"},onMouseDown:n,onMouseMove:y,onMouseUp:C,onMouseLeave:C,onTouchStart:n,onTouchMove:y,onTouchEnd:C})}),e.jsx("p",{className:"text-xs text-muted-foreground text-center",children:"Assine acima com o dedo ou caneta"}),e.jsxs("div",{className:"flex gap-2 justify-between",children:[e.jsxs(p,{variant:"outline",size:"sm",onClick:w,children:[e.jsx(ke,{className:"h-4 w-4 mr-1"})," Limpar"]}),e.jsxs("div",{className:"flex gap-2",children:[e.jsx(p,{variant:"ghost",size:"sm",onClick:d,children:"Cancelar"}),e.jsxs(p,{size:"sm",onClick:D,children:[e.jsx(H,{className:"h-4 w-4 mr-1"})," Salvar Assinatura"]})]})]})]})}function qe({rota:t,dataSelecionada:d,onVoltar:b,user:f}){const u=we(),r=u,[o,n]=h.useState(!1),[y,C]=h.useState(""),[w,D]=h.useState([]),[l,g]=h.useState(!1),[x,A]=h.useState("GRUPO A, B, E INFECTANTES, QUIMICOS E PERFURO CORTANTES"),[_,pe]=h.useState("lista"),[z,k]=h.useState(null),[S,L]=h.useState(null),{data:c=[],isLoading:me}=M({queryKey:["rota-clientes",t.id],queryFn:async()=>{const{data:a}=await j.from("rota_clientes").select("id, ordem, coletado, clientes(id, razao_social, nome_fantasia, logradouro, cidade, cnpj, latitude, longitude)").eq("rota_codigo",t.id).order("ordem");return(a??[]).map(i=>({id:i.id,ordem:i.ordem,coletado:i.coletado??!1,cliente:i.clientes})).filter(i=>i.cliente)}}),{data:N=[]}=M({queryKey:["mtrs-rota",t.id,d],queryFn:async()=>{if(!c.length)return[];const{data:a}=await j.from("mtrs").select("id, numero, cliente_id, status, quantidade, unidade, data_emissao, descricao_residuo, acondicionamento, assinatura_gerador, assinatura_transportador, data_baixa").eq("rota_codigo",t.id).eq("data_emissao",d);return a??[]},enabled:c.length>0}),{data:xe=[]}=M({queryKey:["cdfs-rota",t.id,d],queryFn:async()=>{const{data:a}=await j.from("boletins_medicao").select("id, mtr_id, cdf_id, cdf_enviado, numero").in("mtr_id",N.map(i=>i.id));return a??[]},enabled:N.length>0}),{data:X=[]}=M({queryKey:["clientes-select"],queryFn:async()=>{const{data:a}=await j.from("clientes").select("id, razao_social, nome_fantasia, logradouro, cidade").eq("ativo",!0).order("razao_social",{ascending:!0});return a??[]}}),I=new Set(c.map(a=>a.cliente.id)),U=E({mutationFn:async()=>{const a=w.map((s,m)=>({owner_id:f.id,rota_codigo:t.id,rota_id:null,cliente_id:s,ordem:c.length+m+1,frequencia:"semanal",coletado:!1})),{error:i}=await j.from("rota_clientes").upsert(a,{onConflict:"rota_codigo,cliente_id"});if(i)throw i},onSuccess:()=>{u.invalidateQueries({queryKey:["rota-clientes"]}),R.success(`${w.length} clientes adicionados`),n(!1),D([])},onError:a=>R.error(a.message)}),fe=E({mutationFn:async({mtrId:a,peso:i,cliente:s})=>{const m=new Date().toISOString().split("T")[0],v=Me(),T=crypto.randomUUID(),{data:$,error:ee}=await j.from("mtrs").update({status:"baixado",quantidade:i,data_baixa:m}).eq("id",a).select("id, numero, cliente_id, rota_codigo, descricao_residuo, unidade, data_emissao").single();if(ee)throw ee;const{data:he,error:ae}=await j.from("boletins_medicao").insert([{owner_id:f.id,mtr_id:$.id,cliente_id:$.cliente_id,data_coleta:m,peso_coletado:i,unidade:$.unidade||"kg",status:"pendente",pagamento_confirmado:!1,cdf_enviado:!1,cdf_id:T,numero:v}]).select("id").single();if(ae)throw ae;return{mtrData:$,boletimData:he,numeroCDF:v,cliente:s}},onSuccess:({mtrData:a,numeroCDF:i,cliente:s})=>{r.invalidateQueries({queryKey:["mtrs-rota"]}),r.invalidateQueries({queryKey:["cdfs-rota"]}),R.success("MTR baixado! Boletim e CDF gerados.");const m=new Date().toISOString().split("T")[0],v=re({numeroCDF:i,numeroMTR:a.numero,dataEmissao:m,periodoInicio:a.data_emissao||m,periodoFim:m,cliente:s});L({blobUrl:v,numeroCDF:i})},onError:a=>R.error(a.message)}),Z=E({mutationFn:async({mtrId:a,campo:i,dataUrl:s})=>{const{error:m}=await j.from("mtrs").update({[i]:s}).eq("id",a);if(m)throw m},onSuccess:()=>{r.invalidateQueries({queryKey:["mtrs-rota"]}),R.success("Assinatura salva!")}}),ue=E({mutationFn:async a=>{const{error:i}=await j.from("rota_clientes").delete().eq("id",a);if(i)throw i},onSuccess:()=>{u.invalidateQueries({queryKey:["rota-clientes"]}),R.success("Cliente removido")}}),ge=E({mutationFn:async({rcId:a,coletado:i})=>{const{error:s}=await j.from("rota_clientes").update({coletado:i}).eq("id",a);if(s)throw s},onSuccess:()=>u.invalidateQueries({queryKey:["rota-clientes"]})}),G=E({mutationFn:async()=>{const a=c.map((s,m)=>({owner_id:f.id,cliente_id:s.cliente.id,numero:`MTR-${d.replace(/-/g,"")}-${String(m+1).padStart(3,"0")}`,data_emissao:d,descricao_residuo:x,quantidade:0,unidade:"kg",acondicionamento:"BOMBONA",status:"emitido",rota_codigo:t.id})),{error:i}=await j.from("mtrs").insert(a);if(i)throw i},onSuccess:()=>{u.invalidateQueries({queryKey:["mtrs-rota"]}),u.invalidateQueries({queryKey:["mtrs"]}),R.success(`${c.length} MTRs criados!`),g(!1)},onError:a=>R.error(a.message)}),ve=()=>{const a=window.open("","_blank");if(!a)return;const i=new Date(d+"T12:00:00").toLocaleDateString("pt-BR");a.document.write(`<!DOCTYPE html><html><head><title>Rota ${t.label}</title>
    <style>
