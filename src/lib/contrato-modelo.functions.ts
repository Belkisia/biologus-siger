...
// =============================
export function renderTemplate(html: string, vars: Record<string, string | number | null | undefined>) {
  return html.replace(/\{\{\s*([A-Z0-9_]+)\s*\}\}/g, (_, key) => {
    const v = vars[key];
    if (v === null || v === undefined || v === "") return `<span style="color:#b91c1c">[${key}]</span>`;
    return String(v);
  });
}
...
export function buildVars(args: {
  ...
  const e = args.empresa || {
    ...
    endereco: "Rua Iporá, nº 258, Qd. 18, Lt. 12, Bairro Nossa Senhora de Fátima - Goiânia-GO, CEP 74.420-290",
...
<tbody>${itens.map((i) => `<tr><td>${i.descricao || ""}</td><td>${i.grupo_residuo || "—"}</td><td>${i.unidade || "kg"}</td><td>${i.franquia != null ? `${i.franquia} ${i.unidade || "kg"}` : "—"}</td><td>${brl(Number(i.preco_unitario))}</td><td>${i.preco_excedente ? brl(Number(i.preco_excedente)) : "—"}</td></tr>`).join("")}</tbody></table>`
  ...
  return {
    ...
    CLIENTE_ENDERECO: [c.endereco, c.numero && `nº ${c.numero}`, c.bairro].filter(Boolean).join(", "),
    ...
    FREQUENCIA_COLETA: ct.periodicidade_reajuste === "mensal" ? "mensal" : (ct.frequencia_coleta || "mensal (1 vez ao mês)"),
...
export const atualizarModelo = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id: string; nome: string; descricao?: string | null; conteudo_html: string; motivo?: string }) =>
    z.object({
      id: z.string().uuid(),
      nome: z.string().trim().min(1).max(200),
      descricao: z.string().max(1000).optional().nullable(),
  ...
  .handler(async ({ data, context }) => {
    const { data: atual, error: e1 } = await context.supabase
      .from("contrato_modelos").select("conteudo_html, versao_atual").eq("id", data.id).single();
...
export const duplicarModelo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: src, error } = await context.supabase
      .from("contrato_modelos").select("nome, descricao, conteudo_html").eq("id", data.id).single();
    if (error) throw new Error(error.message);
    ...
    await context.supabase.from("contrato_modelo_versoes").insert({
      modelo_id: novo!.id, versao: 1, conteudo_html: src!.conteudo_html,
      motivo: "Cópia de outro modelo", alterado_por: context.userId,
    });
    return novo;
  });
...
export const alternarAtivoModelo = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id: string; ativo: boolean }) => z.object({ id: z.string().uuid(), ativo: z.boolean() }).parse(d))
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("contrato_modelos").update({ ativo: data.ativo }).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const gerarContratoDeModelo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: m, error: em } = await supabaseAdmin.from("contrato_modelos").select("conteudo_html").eq("id", data.modelo_id).single();
    ...
    if (data.proposta_id) {
      await supabaseAdmin.from("propostas").update({ contrato_id: novo!.id, status: "convertida" }).eq("id", data.proposta_id);
    }
    return { id: novo!.id };
  });
